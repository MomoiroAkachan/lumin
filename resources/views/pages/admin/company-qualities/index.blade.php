    ...
    <div class="bg-interface rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
            <thead class="bg-surface text-left">
                <tr><th class="px-4 py-3">Título</th><th class="px-4 py-3">Ordem</th><th class="px-4 py-3">Status</th><th class="px-4 py-3 text-right">Ações</th></tr>
            </thead>
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                @forelse($qualities as $quality)
                    <tr>
                        <td class="px-4 py-3 font-medium">{{ $quality->title }}</td>
                        <td class="px-4 py-3">{{ $quality->position }}</td>
                ...
                @empty
                    <tr><td colspan="4" class="px-4 py-8 text-center text-gray-500">Nenhuma qualidade cadastrada.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    <div class="mt-4">{{ $qualities->links() }}</div>
@endsection
