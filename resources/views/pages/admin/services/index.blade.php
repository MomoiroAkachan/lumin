...
@section('admin-content')
    ...
    <div class="bg-interface rounded-lg border border-interface-bd  overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200 dark:border-dvide-bd text-sm">
            ...
            <tbody data-sortable-list data-resource="services" class="divide-y divide-gray-200 dark:border-dvide-bd">
                @forelse($services as $service)
                    <tr data-sortable-item data-id="{{ $service->id }}">
                        <x-admin.sortable-handle />
                        <td class="px-4 py-3 font-medium">{{ $service->title }}</td>
                        <td class="px-4 py-3 text-gray-500">{{ $service->slug }}</td>
                        <td class="px-4 py-3"><span data-sortable-position>{{ $service->position }}</span></td>
                        <td class="px-4 py-3">
