    ...
    @else
        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            @foreach($images as $image)
                <div class="relative group bg-interface rounded border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <img src="{{ media_url($image->image_path) }}" class="w-full h-32 object-cover">
                    <div class="p-2 text-xs">
                        <p class="truncate">{{ $image->caption ?: '—' }}</p>
                        <p class="text-gray-400">Ordem: {{ $image->position }} · {{ $image->is_active ? 'Ativo' : 'Inativo' }}</p>
                    </div>
